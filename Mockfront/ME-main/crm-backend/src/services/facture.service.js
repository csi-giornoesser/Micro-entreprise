// crm-backend/src/services/facture.service.js

// --- Rendez l'import du pool robuste à plusieurs formes d'export ---
import * as DB from "../config/db.js";

function resolvePool() {
  // cas 1: export nommé { pool }
  if (DB?.pool && typeof DB.pool.connect === "function") return DB.pool;

  // cas 2: export par défaut = pool
  if (DB?.default && typeof DB.default.connect === "function") return DB.default;

  // cas 3: export par défaut = objet { pool }
  if (DB?.default?.pool && typeof DB.default.pool.connect === "function") {
    return DB.default.pool;
  }

  // cas 4: helper getPool()
  ...
  throw new Error(
    "Impossible de résoudre le pool Postgres depuis src/config/db.js. " +
    "Assure-toi d'exporter soit { pool }, soit default pool, soit getPool()."
  );
}
...
function periodBounds(periode /* "YYYY-MM" */) {
  ...
  if (!y || !m || m < 1 || m > 12) {
    throw new Error("periode doit être au format YYYY-MM");
  }
  const start = new Date(Date.UTC(y, m - 1, 1));
  const end   = new Date(Date.UTC(m === 12 ? y + 1 : y, m === 12 ? 0 : m, 1)); // exclusif
...
// ------------------------------ service principal ------------------------------
/**
 * Calcule ou crée une facture partenaire sur la base des dossiers
 * payés (e.service_paiement->>'statut' = 'Payé') dans la période.
 *
 * @param {object} params
 * @param {number} params.partnerId
 * @param {string} params.periode  ex: "2025-07"
 * @param {number|null} params.rate  ex: 0.15 (15%) ; si null → lit partenaires.taux_commission
 * @param {boolean} params.dryRun  si true → ne persiste pas (preview)
 */
export async function previewOrCreateInvoice({ partnerId, periode, rate, dryRun = true }) {
  ...
  try {
    ...
    if (effRate == null) {
      ...
      // partenaires.taux_commission est en %, on convertit en ratio
    ...
    // Statut/date/échéance selon si période passée
